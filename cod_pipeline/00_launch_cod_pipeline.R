...
system(paste("qsub -P PROJECT -l m_mem_free=5G -l fthread=2 -q all.q  -N", pool_job_name,
...
for (cause in causes){
  for (sex_id in sex_ids){
    ...
    qsub_command <- paste("qsub -hold_jid", interp_hold, "-P PROJECT -l m_mem_free=110G -l fthread=15 -q long.q  -N", save_script_name, 
                          "-e FILEPATH", "-o FILEPATH", "FILEPATH", "-s ",
...
for (cause in causes){
  for (sex_id in sex_ids){
    ...
    system(paste("qsub -P PROJECT -l m_mem_free=90G -l fthread=15 -q all.q  -N", save_script_name, 
